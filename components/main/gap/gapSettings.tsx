...
import { AlertCircle, Loader2, FileText, Zap, Info, CheckCircle } from "lucide-react"
import { gapAnalysis } from "@/actions/ai.actions"
      ...
      <Tabs defaultValue="skills-gaps" className="w-full mt-8">
              ...
              {analysisResult.skillsAndExperienceGaps.map((gap, index) => (
                <div key={index} className="mb-4 p-4 border rounded-md">
                  <div className="flex justify-between items-center">
                    ...
                    <Badge
                      variant={
                        gap.severity === "Critical" ? "destructive" : gap.severity === "High" ? "default" : "secondary"
                      }
                    >
                      {gap.severity}
                    </Badge>
                    ...
                    <AccordionItem value={`gap-${index}`}>
                      ...
                      <AccordionContent>
                        <p className="text-sm mt-1">
                          <strong>Job Requirement:</strong> {gap.jobDescriptionExcerpt}
                        </p>
                        <p className="text-sm mt-1">
        ...
        <TabsContent value="resume-improvements">
          <Card>
            ...
            <CardContent>
              {analysisResult.resumeImprovementSuggestions.map((suggestion, index) => (
                <div key={index} className="mb-4 p-4 border rounded-md">
                  ...
                  <Accordion type="single" collapsible className="w-full">
                    <AccordionItem value={`suggestion-${index}`}>
                      ...
                      <AccordionContent>
                        <div className="mt-2">
                          <p className="text-sm">
                            <strong>Before:</strong> {suggestion.example.before}
                          </p>
                          <p className="text-sm mt-1">
                            <strong>After:</strong> {suggestion.example.after}
                          </p>
                          <p className="text-sm mt-1">
                            <strong>Rationale:</strong> {suggestion.example.rationale}
                          </p>
                        </div>
                      </AccordionContent>
                    </AccordionItem>
    ...
    <Card className="lg:col-span-2 h-fit">
      ...
      <CardContent className="space-y-8">
        <div className="grid gap-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="resumeSelect" className="text-sm font-medium flex items-center gap-2">
              Select Resume
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger>
        ...
        {analysisResult && (
          <Alert>
            ...
            <AlertDescription>
              Your resume has been analyzed against the job description. View the results below.
            </AlertDescription>
          </Alert>
        )}
